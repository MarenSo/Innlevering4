...
mod1 <- 'pm2 ~ aar_f + ya_total + inc_k1 + inc_k5 + uni_k_mf + uni_l_mf + Trade_pc_100K'
```
...
```{r}
mod2 <- 'pm2 ~ aar_f*fnr + ya_total + inc_k1 + inc_k5 + uni_k_mf + uni_l_mf + Trade_pc_100K'
...
scale_size_manual(values = c(seq(2.1, 0, by = -0.1))) +
geom_hline(yintercept = 0) +
theme(legend.position = 'bottom')
```
...
Feil! Modelen undervurderer prisen derfor positiv residual.
...
```{r lager pm2_n}
pm2_n <- pm2 %>%
  select(pm2, fnr, knr, aar_d, aar, aar_f, ya_menn, ya_kvinner, ya_total, inc_k1, inc_k5, uni_k_mf, uni_l_mf, Trade_pc_100K) %>%
  group_by(aar_d) %>%
  nest()
```
...
kom_model <- function(a_df) {
  lm(pm2 ~ fnr + ya_total + inc_k1 + inc_k5 + uni_k_mf + uni_l_mf + Trade_pc_100K, data = a_df)
}
```
...
select(aar, variables, coef) %>%
filter(
variables %in% c("ya_total", "inc_k1", "inc_k5", "uni_k_mf", "uni_l_mf", "Trade_pc_100K")
) %>%
ggplot(mapping = aes(x = aar, y = coef, colour = variables)) +
scale_color_viridis(discrete = TRUE, option = "D") +
geom_line(aes(group = variables), lwd = 1) +
