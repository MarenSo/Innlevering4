...
# Mye fin kode i de neste chunk-ene, men helt unødvendig. Alt dere
# trenger ligger i ya_prosent ovenfor
# Dette er gjort ovenfor i chunk-en: utvidelse av hele befolkning
# pop_08_17 <- pop_08_17 %>%
...
```{r legge til i pm2 B}
# dere har det dere trenger i ya_prosent
# gjenstår bare å joine pm2 med ya_prosent
pm2 <- pm2 %>%
  inner_join(ya_prosent,
            by = c("knr", "aar"))
```
...
inc_k_08_17 <- inc_k_08_17_raw %>% 
  pivot_wider(
    names_from = Desiler,
    # R liker ikke varaibelnavn som starter med tall, derfor "d"
    names_prefix = "d",
    values_from = value
    )
```
...
# Her legger dere bare inn faste verdier. Det går galt
...
# Langt enklere å bruke rename() i en pipe
...
#Ferdig
```{r sjekke pm2 A}
# Neste er kommentert ut. Gir ca. 100 sider med datautskrift
#na.omit(pm2)
dim(pm2)
```
...
pm2 <- pm2 %>% select (knr,knavn,aar,pm2,ya_menn,ya_kvinner,ya_total,inc_k1,inc_k5,uni_k_mf,uni_k_m,uni_k_f,uni_l_mf,uni_l_m,uni_l_f,trade_pc)
```
...
```
```{r lage csv}
# Bruk relative stier til filer. Ellers virker det ikke på andre maskiner
# Bruk heller write_csv() fra tidyverse. Det er en bedre funksjon.
